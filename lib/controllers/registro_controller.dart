...
// Provider de sensaciones últimos 7 días: { counts: Map<int,int>, average: double }
final sensacionUltimos7Provider =
  ...
  final registros = await repository.getByDateRange(start, end);

  final Map<int, int> counts = {1: 0, 3: 0, 5: 0};
  int sum = 0;
  int n = 0;
...
// Provider de sensaciones de HOY: { counts: Map<int,int>, average: double, total: int }
final sensacionHoyProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  final repository = ref.watch(registroRepositoryProvider);
  final now = DateTime.now();
...
final registrosPorEjercicioProvider =
    FutureProvider.family<List<RegistroEjercicio>, int>(
        (ref, ejercicioId) async {
  final repository = ref.watch(registroRepositoryProvider);
  return await repository.getByEjercicio(ejercicioId);
});

// Provider de estadísticas por ejercicio
final estadisticasEjercicioProvider =
    FutureProvider.family<Map<String, dynamic>, int>((ref, ejercicioId) async {
  final repository = ref.watch(registroRepositoryProvider);
...
class RegistroController extends StateNotifier<AsyncValue<void>> {
  ...
  Future<List<RegistroEjercicio>> obtenerRegistrosPorFecha(
...
final registroControllerProvider =
    StateNotifierProvider<RegistroController, AsyncValue<void>>((ref) {
  final repository = ref.watch(registroRepositoryProvider);
